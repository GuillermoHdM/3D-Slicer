...
memcheck-suppressions:
	valgrind --quiet --leak-check=full --leak-resolution=med --track-origins=yes --gen-suppressions=all --log-file=minimalraw.log --error-exitcode=-1 --vgdb=no $(TEST-EXE) --gtest_filter=*
	
memcheck: 
	@#valgrind --quiet --leak-check=full --leak-resolution=med --track-origins=yes --suppressions=opengl.valgrind.supp --error-exitcode=-1 --vgdb=no $(TEST-EXE) --gtest_filter=*
